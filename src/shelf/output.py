...
def write_shelf(tree: BookTree, output_dir: Path) -> None:
    ...
    for ch_idx, (ch_slug, ch_dir, chapter) in enumerate(chapter_dirs, start=1):
        ...
        index_lines.append(f"- [{chapter.title}]({ch_prefix}/README.md)")
        for sec_idx, section in enumerate(chapter.children, start=1):
            sec_slug = slugify(section.title) or f"section-{sec_idx}"
            sec_prefix = f"{sec_idx:02d}-{sec_slug}"
            index_lines.append(f"  - [{section.title}]({ch_prefix}/{sec_prefix}.md)")
            # H3+ as anchor links
